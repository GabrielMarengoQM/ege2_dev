...
box::use(
  shiny[moduleServer, NS, reactive,
        ...
        tagList, renderText, textOutput, isolate, p, showModal,
        modalDialog],
...
#' @export
server <- function(id, saved_lists_and_filters, edited_list, data, nice_col_names, filter_differences, vars, save_filters, clear_filters, current_genes) {
  moduleServer(id, function(input, output, session) {
    ...
    #     tabPanel("Constraint metrics (cell lines)",
    ...
    #     tabPanel("Constraint metrics (population sequencing)",
    ...
    output$filter <- renderUI({
      ns <- NS(id)
      accordion(
        id = session$ns("filter_accordion"),
        accordion_panel(
          title = "Gene symbol/HGNC ID",
          actionButton(session$ns("gene_modal"), "Filters info"),
          imap(vars()[1:4], ~filters_utils$make_ui(data[[.x]], .x, nice_col_names[[.y]], id, session))
        ),
        accordion_panel(
        ...
        accordion_panel(
          title = "Constraint metrics (cell lines)",
          actionButton(session$ns("constraint_cell_modal"), "Filters info"),
          imap(vars()[22:25], ~filters_utils$make_ui(data[[.x]], .x, nice_col_names[[.y + 21]], id, session))
        ),
        accordion_panel(
          title = "Constraint metrics (population sequencing)",
          actionButton(session$ns("constraint_pop_seq_modal"), "Filters info"),
          imap(vars()[26:32], ~filters_utils$make_ui(data[[.x]], .x, nice_col_names[[.y + 25]], id, session))
        ),
        accordion_panel(
    ...
    # render datatable ----
    observe({
      tbl <- renderTable(head(data[selected(), c('gene_symbol', 'impc_viability', 'gnomad_lof_upper_90_ci', 'phenotypes')], 12))
      current_genes(tbl)
    })
    ...
    # # save filters ----
    observeEvent(save_filters(), {
      # Get the current list name
      list_name <- save_filters()
      # req(list_name != "")
    ...
    #   gene_list <- 'ABC'
    #   saved_data <- list('gene_list' = unique(gene_list), 'filters' = current_filters())
    #   list_to_save(saved_data)
    #
    # })

    # reset filters ----
    observeEvent(clear_filters(), {
    ...
    filter_differences_val <- reactive({
      map2(
    ...
    observe({
      filter_differences_val()
      filter_differences(filter_differences_val())
    })
